...
<div class="container">	
 <div class="row"> 
  ...
  // Akcja przypisywania zaznaczonych rekordów (unikatowych) do tabeli polacz	
	if ($ile1 > 0 && $ile2 > 0 ) 
	{	
		for ($i=1; $i<=$ile1; $i++)
		{
  ...
  //Spr bilingu w tabeli nie_przyp i przepisanie pola do polaczenia i kasacja w nie_przyp
	...
	$lista_polaczenia = $db -> query('select id_p, id_bili, data, godzina_polaczenia, czas_trwania_polaczenia, kierunek_polaczenia, numer_telefonu from nie_przyp;');
  ...
  //Zapytanie do tabeli nie_przyp 
	$lista_nieprzyp = $db -> query('select id_p, id_bili, data, godzina_polaczenia, czas_trwania_polaczenia, kierunek_polaczenia, numer_telefonu from nie_przyp;');

  // Formularz z polem select - dane z tabeli nie_przyp
  
	echo '<form class="form-inline" role="form" method="POST" action="przypis.php"> 
		<select multiple name="nazwa[]" size="30">';
		
		foreach ($lista_nieprzyp as $linia)
			{
				$wiersz++;
				echo '<option value='.$linia["id_bili"].'>'.$linia["id_bili"].' '.$linia["data"].' '.$linia["godzina_polaczenia"].' '.$linia["czas_trwania_polaczenia"]. ' ' .$linia["kierunek_polaczenia"]. ' ' .$linia["numer_telefonu"] . '</option>' ;
			}
		echo '</select><br/>';
